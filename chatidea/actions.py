...
def extract_entities(entities: list[extractor.Entity], entity_name: str) -> \
                ...
                if attr is None and attribute_name in order_by_alias:
                    ...
                    if columns_element:
                        ...
                        response.add_message(
                            'Sorry, there is no attribute "' + columns + '" for ' + element_name)
                        return []
...
def get_attributes_string(attributes):
    return ', '.join(
        ('{} '.format(a.get('keyword')) if a.get('keyword') else '')
...
def handle_response_for_quantity_found_element(response, element, context):
    if element['real_value_length'] == 1:
        ...
        response.add_message(
            msg.N_RESULTS_FOUND_PATTERN.format(element['real_value_length']))
        if element.get('action_type') != 'filter' and \
                element.get(
                    'element_name') in resolver.get_all_primary_element_names():
    ...
    if element_name:
        ...
        if category:
            response.add_buttons(
                btn.get_button_show_table_categories(element_name))
        # response.add_button(btn.get_button_help_on_elements())


    else:
        response.add_message(
            'I am sorry, I understood that you want more info, but not on what...')
    response.add_button(btn.get_button_help_on_elements())
...
def action_more_info_filter(entities, response, context):
    ...
    if element_name:
        ...
        if element and element['real_value_length'] > 1:
            response.add_message(
                msg.filter_element_examples(element['element_name']))
        else:
            response.add_message(
                'I am sorry, but there is nothing to filter... You\'d better tell on which element.\n'
...
def action_ambiguity_solver(entities: list[extractor.Entity], response,
    ...
    if not contain(entities, nlu.ENTITY_WORD) and not contain(entities,
                                                              nlu.ENTITY_NUMBER):
    ...
    if contain(entities, nlu.ENTITY_ELEMENT) and el_number == word_el_number:
        ...
        all_similar_words = resolver.extract_similar_values(
            find_word_numbers(entities))
        remove_el(entities)
        for word in all_similar_words:
            new_entities = entities.copy()
            replace_word_numbers(new_entities, word)
            autocompleter.autocomplete_from_word(new_entities, response,
                                                 context)
...
def action_find_element_by_attribute(entities: list[extractor.Entity],
                                     response, context):
    element_name = handle_element_name_similarity(
        extract_single_entity_value(entities, nlu.ENTITY_ELEMENT))
    ...
    if element_name:
        attributes = get_attributes_from_ordered_entities(element_name,
                                                          ordered_entities,
                                                          response) if ordered_entities else []
        if attributes:
            element = resolver.query_find(element_name, attributes)
            if element['value']:
                element[
                    'action_name'] = '...found with attribute(s) "{}"'.format(
                    get_attributes_string(attributes))
            ...
            if response.isEmpty():
                response.add_message(
                    'Ok so you want to find some concepts of type {}, but you should '
                    'tell me something more, otherwise I can\'t help you explore!'.format(
                        element_name))
                response.add_message(msg.find_element_examples(element_name))
                response.add_button(btn.get_button_help_on_elements())
                # response.add_message('Remember that you can always go back, just click the button')
    ...
    if element:
        ...
        if element['real_value_length'] > 1:
            ...
            attributes = get_attributes_from_ordered_entities(
                element['element_name'], ordered_entities, response)

            if attributes:
                # here down union of attributes
                all_attributes = copy.deepcopy(
                    element['attributes']) if element.get('attributes') else []
                if all_attributes[-1][
                    'keyword'] == 'order by':  # 'and' the last where in the context with the first of the filter
                    all_attributes[-1]['and_or'] = 'and'
                else:
                    all_attributes[-1]['and_or'] = 'and'
                all_attributes += copy.deepcopy(attributes)
                result_element = resolver.query_find(element['element_name'],
                                                     all_attributes)
                if result_element['value']:
                    result_element[
                        'action_name'] = '...by filtering with property(s) ' \
                                         '"{}":'.format(
                        get_attributes_string(attributes))
                    result_element['action_type'] = 'filter'
                    context.append_element(result_element)
                    handle_response_for_quantity_found_element(response,
                                                               result_element,
                                                               context)
                    action_view_context_element(entities, response, context)

                else:
                    error_message = "Nothing as been found for {} ".format(
                        element['element_name'])
                    error_message += ", ".join(
                        ["{} {}".format(e['attribute'], e['value']) for e in
                         ordered_entities])
                    ...
                    # response.add_message('Remember that you can always go back, just click the button')
                    response.add_button(
                        btn.get_button_go_back_to_context_position(
                            '- GO BACK! -',
                            len(context.get_context_list()) - 1))
                    response.add_button(btn.get_button_history())
                ...
                response.add_message(
                    "I didn't understand for what do you want to filter by\n")
                action_more_info_filter(entities, response, context)

        else:
            response.add_message(
                'Filtering is not possible now, there is only one element under to view!')
            response.add_button(btn.get_button_help_on_elements())
            # response.add_message('Remember that you can always go back, just click the button')
            response.add_button(
                btn.get_button_go_back_to_context_position('- GO BACK! -',
...
def action_cross_relation(entities, response, context):
    ...
    if element:
        ...
        extracted_relation_name = extract_single_entity_value(entities,
                                                              nlu.ENTITY_RELATION)
        relation_name = handle_element_relations_similarity(
            element['element_name'], extracted_relation_name)
        ...
        if relation_name:
            ...
            if element['real_value_length'] == 1:
                ...
                if result_element['value']:
                    ...
                    result_element[
                        'action_name'] = '...reached with the relation "{}", from {}:' \
                    ...
                    response.add_message(msg.NOTHING_FOUND)
                    response.add_button(btn.get_button_help_on_elements())
                    response.add_button(btn.get_button_view_context_element(
...
def action_show_relations(entities, response, context):
    ...
    if element:
        ...
        if buttons:
            response.add_message(
                'If you want more information, I can tell you:')
            ...
            if element['real_value_length'] == 1:
                ...
                if is_selection_valid(element, position):
                    if is_value_in_selection_valid(element, position, title):
                        add_selected_element_to_context(element, position,
                                                        context)
                        action_view_context_element(entities, response,
                                                    context)
                    else:
                        response.add_message(
                            'Error! The selected element not belonging to the context!')
                        response.add_button(btn.get_button_help_on_elements())
                        response.add_button(
                            btn.get_button_go_back_to_context_position(
                                '- GO BACK! -',
...
def action_view_context_element(entities, response, context, show_less=False):
    ...
    if element:
        if element['element_name'] == "more_info_find":
            action_more_info_find(element['entities'], response, context,
                                  False)
            return
        ...
        if element['element_name'] == "show_table_categories":
            action_show_table_categories(element['entities'], response,
                                         context, False)
            return

        if element['real_value_length'] == 1:
            if previous_element:
                if element['element_name'] == previous_element[
                    'element_name'] and previous_element[
                    'real_value_length'] > 1:
                    previous_results_list = ''
                    showed_elements = previous_element['show']['to']
                    for i in previous_element['value'][:showed_elements]:
                        previous_results_list += '- ' + resolver.get_element_show_string(
                            previous_element['element_name'], i) + '\n'
            ...
            if element['show']['from'] == 0 and not show_less:
                # response.add_message('Remember that you can always go back, just click the button')
                # response.add_button(btn.get_button_go_back_to_context_position('- GO BACK! -', len(context.get_context_list()) - 1))
                response.add_message(msg.SELECT_FOR_INFO_PATTERN.format(
                    element.get('element_name')))
            response.add_message('Shown results from {} to {} of {}'.format(
                element['show']['from'] + 1,
                element['show']['to'],
...
def action_show_more_elements(entities, response, context):
    ...
    if element and element['real_value_length'] > 1:
        if element['show']['to'] < element['real_value_length']:
            element['show']['from'] = element['show'][
                                          'from'] + ELEMENT_VISU_LIMIT
            element['show']['to'] = min(element['real_value_length'],
                                        element['show'][
                                            'to'] + ELEMENT_VISU_LIMIT)
            context.reset_show_last_element = False
            action_view_context_element(entities, response, context)
        else:
            response.add_message(
                'I am sorry, but there is nothing to show more...')
    else:
        response.add_message(
            'I am sorry, but there is nothing to show more...')
...
def action_show_less_elements(entities, response, context):
    element = context.get_last_element()
    if element and element['real_value_length'] > 1 and element['show'][
        'to'] > ELEMENT_VISU_LIMIT:
        diff = 0
        if element['show']['to'] == element['real_value_length']:
        ...
        element['show']['from'] = max(0, element['show'][
            'from'] - ELEMENT_VISU_LIMIT)
        context.reset_show_last_element = False
        action_view_context_element(entities, response, context,
                                    show_less=True)
        ...
        response.add_message(
            'I am sorry, but there is nothing to show less...')
...
def action_order_by(entities, response, context):
    ...
    if element:
        ...
        # response.add_button(btn.get_button_help_on_elements())
        response.add_buttons(
            btn.get_buttons_order_by_attribute(value_attributes, element_name))
    else:
        # response.add_button(btn.get_button_help_on_elements())
...
def action_order_by_attribute(entities, response, context):
    attribute_to_order_by = extract_single_entity_value(entities,
                                                        nlu.ENTITY_POSITION)
    element = context.get_last_element()
    value = element['value']
    value = sorted(value, key=lambda k: (k[attribute_to_order_by] is None, k[
        attribute_to_order_by]))  # All None element are put in the end
    element['value'] = value
    context.reset_show_last_element = False
    action_view_context_element(entities, response, context)
...
def action_show_more_examples(entities, response, context):
    ...
    response.add_message(
        'Select the property of "' + element_name + '" you want to see examples of')
    response.add_buttons(
        btn.get_buttons_show_more_ex_attr(element_name, attributes))
...
def action_show_context(entities, response, context):
    ...
    if context_list:
        ...
        for i, el in enumerate(context_list[down:up][
                               ::-1]):  # veramente brutto TODO sistemare con uno switch in caso di avanzo di tempo
            ...
            if el['element_name'] == "more_info_find":
                ...
                if el['element_name'] == "start":
                    ...
                    if el['element_name'] == "show_table_categories":
                        result = 'Categories of "{}"'.format(
                            el['element_value'])
                    else:
                        if el['real_value_length'] == 1:
                            result = '{}'.format(
                                resolver.get_element_show_string(
                                    el['element_name'], el['value'][0]))
                        else:
                            result = 'A list of type "{}"'.format(
                                el['element_name'])
            ...
            if up == len(context_list) and i == 0:  # the very first one
                ...
                response.add_message(
                    "Here is history, click on a button to see the related element")
                response.add_button(
                    btn.get_button_view_context_element(result))
                ...
                response.add_button(
                    btn.get_button_go_back_to_context_position(result, up - i))
...
def action_show_more_context(entities, response, context):
    ...
    if context_list:
        if context.context_list_indices['down'] != 0:  # if it is not 0
            context.context_list_indices['down'] = max(
                context.context_list_indices['down'] - CONTEXT_VISU_LIMIT, 0)
            context.context_list_indices['up'] = context.context_list_indices[
                                                     'up'] - CONTEXT_VISU_LIMIT
...
def action_go_back_to_context_position(entities, response, context):
    ...
    if context.get_context_list():
        ...
        if position == nlu.VALUE_POSITION_RESET_CONTEXT or \
            ...
            response.add_button(
                btn.get_button_help_on_elements())  # response.add_message('Remember that you can always go back, just click the button')
            response.add_button(
                btn.get_button_go_back_to_context_position('- GO BACK! -',
                                                           len(context.get_context_list()) - 1))
            response.add_button(btn.get_button_history())
            ...
            response.add_message(
                'Ok, now resuming your history of {} position{}... DONE!'
                .format(length - position,
                        "s" if length - position > 1 else ""))
            action_view_context_element(entities, response, context)
...
def action_find_element_by_category(entities, response, context):
    ...
    if element:
        ...
        if element_name and category_value:
            ...
            if element['value']:
                element[
                    'action_name'] = '...found from category {0} of table {1}'.format(
                    category_value, element_name.upper())
                element['action_type'] = 'find'
                context.append_element(element)
                # handle_response_for_quantity_found_element(response, element, context)
                action_view_context_element(entities, response, context)
...
def action_show_table_categories(entities, response, context, add=True):
    ...
    if element_name:
        if category:
            ...
            response.add_message(
                'I cannot find more info about {}s.'.format(element_name))
...
def execute_action_from_intent_name(intent_name: str,
                                    entities: list[extractor.Entity], context):
